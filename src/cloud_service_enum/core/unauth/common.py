"""Shared scaffolding for ``cse <provider> unauth <service>`` runners.
...
public cloud endpoints, then an :class:`EnumerationRun` built on top of
the shared display helpers. This module holds the bits that don't vary
between providers so each per-service runner stays short.
"""
...
# The CLI decorator in ``clis/common.py`` mirrors these so that behaviour
# stays aligned across every provider's ``unauth`` sub-tree.
...
def build_identity(provider: Provider, label: str) -> dict[str, Any]:
...
def render_preamble(
    ...
    *,
    ...
    identity = build_identity(provider, identity_label)
    render_identity(console, identity)
    render_config(console, provider, cse_scope, extras=extras or {})
